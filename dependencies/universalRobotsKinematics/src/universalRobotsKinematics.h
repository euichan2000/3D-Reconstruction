...
namespace universalRobots
{
	...
	public:
		...
		void loadYAML(const std::string &filename, float &thetaX, float &thetaY, float &thetaZ, float &X, float &Y, float &Z, float scene[][6], int &n, float minrange[4], float maxrange[4], float &downsampleparam, int &sor_mean, float &sor_thresh, float &ror_radius, int &ror_neighbor);
