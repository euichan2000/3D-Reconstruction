...
int main(int argc, char **argv)
{
  ...
  *cloud = *data[0].cloud;
  ...
  for (const auto &cluster : cluster_indices)
  {
    ...
    std::string clusterFileName = "./segmented_pcd/mustard/segmented_" + std::to_string(j) + ".pcd";

    // writer.write<pcl::PointXYZ>(clusterFileName, *cloud_cluster, false);
    pcl::io::savePCDFile(clusterFileName, *cloud_cluster, true);
